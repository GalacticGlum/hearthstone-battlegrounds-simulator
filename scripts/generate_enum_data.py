...
def write_enum_data_cpp(minions, heroes):
  with open("src/enum_data.cpp","wt", encoding="utf-8") as f:
    ...
    f.write("#include \"random_keys.hpp\"\n")
    ...
    # Generate a map that maps minion names (string) to minion types
    f.write("// -----------------------------------------------------------------------------\n")
    f.write("// Minion name-type map\n")
    f.write("// -----------------------------------------------------------------------------\n\n")
    f.write('const std::unordered_map<std::string, MinionType> minion_name_type_map({\n')
    for m in minions:
      e = m[0]
      f.write('{{ "{}", MinionType::{} }},\n'.format(e.name, e.enum))
    f.write('});\n')
...
def write_minion_list(f, minions, name, filter):
  ...
  for m in minions:
    ...
    if filter(e):
      f.write("  MinionType::{},\n".format(e.enum))
  f.write("}};\n".format(name))
  f.write("MinionType random_{}_minion(BattleRNG& rng, int player) {{\n".format(name))
  f.write("  return random_element({}_minions, rng, rng_key(RNGType::{}Minion, player));\n".format(name,enum_name(name)))
  f.write("}\n\n")
